...
to go
  ...
  if dirt-probability >= 85
                         ;; ako je veci ili jednak od 85 (15%)
  [
...
to clean
  let dirty-patches (patches in-radius 20 with [pcolor = gray])
                                        ;; VARIJABLA DIRTY-PATCHES SE POSTAVLJA NA LISTU PRLJAVIH PATCHEVA UNUTAR RADIJUSA OD 20 JEDINICA OD TRENUTNE POZICIJE USISIVAČA
  if any? dirty-patches [               ;; PROVJERAVA SE POSTOJANJE PRLJAVIH PATCHEVA UNUTAR RADIJUSA
    let target-patch min-one-of dirty-patches [distance myself]
                                        ;; ODABIRE SE NAJBLIŽI PRLJAVI PATCH IZ LISTE, KORISTEĆI MIN-ONE-OF S KRITERIJEM UDALJENOSTI OD TRENUTNE POZICIJE USISIVAČA
    face target-patch                   ;; USISIVAČ SE OKREĆE PREMA ODABRANOM PRLJAVOM PATCHU
    fd 1
    if [pcolor] of patch-here = gray [
      set dirt-count dirt-count + 1
      show (word "Dirt: " dirt-count)
      wait 0.3
      set pcolor black
    ]
  ]
