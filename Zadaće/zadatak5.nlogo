...
to go
  ...
  if (length samples1 = 2) and (empty? model) [
    show "Found both Orange and Blue!"
    stop
  ]
...
to walk1  ;; funkcija prvog rovera
  ...
  [
    ...
    [
      ...
      set temporary-list lput xcor temporary-list
      set temporary-list lput ycor temporary-list
      set model lput temporary-list model
      show samples1
    ]
  ]
...
if not empty? model
  [
    set closest-coordinates(item 0 model)
    set min-distance 9999
    set i 0
    foreach model
   [
    xy ->  if distance(patch item 0 xy item 1 xy) < min-distance
     [
      set min-distance distance(patch item 0 xy item 1 xy)
  ...
  set current-x (item 0 closest-coordinates)
  set current-y (item 1 closest-coordinates)
  facexy current-x current-y
  ifelse (distancexy current-x current-y) > 1
   [
    fd 1
   ]
   [
    fd distancexy current-x current-y
   ]

  if pxcor = current-x and pycor = current-y
   [
   set model remove-item index model
   show (ifelse-value (pcolor = blue) ["Found Blue!"] ["Found Orange!"])
   set pcolor black
   ]
  ]
end
