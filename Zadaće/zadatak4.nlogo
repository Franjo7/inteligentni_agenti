globals [new-products products badx bady ticksCounter valid-products invalid-products]
                                   ;; DODANA VARIJABLA ticksCounter ZA BROJAC OTKUCAJA
                                   ;; DODANE VARIJABLE valid-products i invalid-products ZA BROJANJE ISPRAVNIH I NEISPRAVNIH PROIZVODA
...
to setup
  ...
  set badx 6
  set bady 5
  set ticksCounter 10             ;; POSTAVLJANJE BROJACA OTKUCAJA
  set valid-products 0            ;; POSTAVLJANJE BROJACA ISPRAVNIH PROIZVODA
  set invalid-products 0          ;; POSTAVLJANJE BROJACA NEISPRAVNIH PROIZVODA
...
to go
  ...
  if (valid-products = 30) or (invalid-products = 3)           ;; UVJET ZAUSTAVLJANJA SIMULACIJE
  [
    ...
    show (word "Valid Products: " valid-products)             ;; ISPIS UKUPNOG BROJA ISPRAVNIH PROIZVODA
    show (word "Invalid Products: " invalid-products)         ;; ISPIS UKUPNOG BROJA NEISPRAVNIH PROIZVODA
    stop
  ]

  if ticks mod ticksCounter = 0                               ;; NAKON SVAKIH 10 OTKUCAJA POJAVLJUJE SE NOVI PROIZVOD NA TRACI
  [
    new-product
  ]
...
to walk
  ...
  [
    ...
    [
      ...
      ifelse color = red                                   ;; UKOLIKO JE PROIZVOD CRVENE BOJE
      [
        set invalid-products invalid-products + 1          ;; POVECAVAMO BROJAC NEISPRAVNIH
        show (word "Invalid Product No. " products)
        setxy badx bady
        ifelse bady = -1
        [
          set badx badx - 1
      ...
      [                                                    ;; UKOLIKO PROIZVOD NIJE CRVENE BOJE
        set valid-products valid-products + 1              ;; POVECAVAMO BROJAC ISPRAVNIH
        show (word "Valid Product No. " products)
        die
      ]
    ]
    [
...
Svaki put kad prođe određeni broj otkucaja, automatski se pojavljuje novi proizvod na traci. Agent registira neispravne proizvode (obojene crvenom bojom) i ispravne proizvode (obojene nekom drugom bojom) kada dođu do kraja proizvodne trake. Ispravni proizvodi se uklanjaju sa ekrana, a neispravni zadržavaju na ekranu.
